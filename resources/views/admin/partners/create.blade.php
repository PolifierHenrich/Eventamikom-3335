...
@section('page_subtitle', 'Isi data mitra atau sponsor baru di bawah ini.')
        ...
        <form action="{{ route('admin.partners.store') }}" method="POST">
            ...
            <div class="mb-8">
                ...
                <textarea id="description" name="description" rows="4"
                          ...
                          class="w-full border border-slate-200 rounded-2xl px-5 py-4 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition @error('description') border-red-400 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
